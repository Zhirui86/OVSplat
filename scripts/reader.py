...
def main():
  file_dir = '../datasets/scans/{}/{}.sens'.format(opt.scene, opt.scene)
  output_path = '../datasets/scannet/{}'.format(opt.scene)
  if not os.path.exists(output_path):
    os.makedirs(output_path)

  if opt.export_label_lift:
    import zipfile
    with zipfile.ZipFile('../datasets/scans/{}/{}_2d-label-filt.zip'.format(opt.scene, opt.scene), 'r') as zip_ref:
